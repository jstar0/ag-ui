...
- `create_strands_app(agent, path="/", ping_path="/ping", origins=None)` bootstraps a FastAPI application, adds `CORSMiddleware`, and mounts the agent route. The middleware is unconditional and open by default: `allow_origins=origins or ["*"]` falls back to the wildcard whenever `origins` is omitted or empty (an empty list is falsy, so `origins or ["*"]` selects the wildcard), `allow_methods` and `allow_headers` are both `["*"]`, and there is no switch for turning CORS off. `allow_credentials` is the one guarded field, computed as `bool(origins) and not is_wildcard`, so credentials are enabled only for an explicit non-wildcard allowlist. The TypeScript factory diverges on the default and on credentials in opposite directions; see [Opt-in cross-origin access](#additions-beyond-the-python-adapter).
