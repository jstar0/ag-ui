...
function stubFetch(handler: (url: string) => Response | Promise<Response>) {
  ...
  const spy = vi.fn(async (input: unknown, init?: RequestInit) => {
    ...
    for (let hop = 0; ; hop++) {
      ...
      if (!isRedirect || init?.redirect === "manual") return res;
      if (hop > 20) throw new Error("stub followed too many redirects");
...
describe("URL fetch policy: redirects", () => {
  ...
  it("names the failing redirect hop, not just the original URL", async () => {
    ...
    expect(String(log.warn.mock.calls[0][0])).toContain(
      "at redirect target http://public.example/gone",
    );
  });
  ...
  it("names the hop when a redirect target is refused on policy", async () => {
    ...
    // The hop is named, but a host-less URL is never echoed in full, so the
    // path is withheld and the scheme carries the diagnosis.
    expect(message).toContain("at redirect target file:");
...
describe("URL fetch policy: configuration", () => {
  ...
  it("allows an extra scheme when opted into", async () => {
    ...
    const bytes = await fetchUrlBytes(
      ...
      makeLog(),
      policy({
        allowedSchemes: new Set(["ftp"]),
        allowPrivateNetworks: true,
      }),
    );

    expect(bytes && Buffer.from(bytes).toString()).toBe("data");
  });

  it("refuses a host-less URL unless private networks are opted into", async () => {
    const dataUrl = "data:text/plain;base64,aGVsbG8=";
    const schemeOnly = policy({ allowedSchemes: new Set(["data"]) });

    await expect(validateFetchUrl(dataUrl, schemeOnly)).rejects.toThrow(
      /no host/,
    );
    await expect(
      validateFetchUrl(
...
describe("URL fetch policy: IPv6 transition forms", () => {
  ...
  it.each([
    // RFC-conformant encoding of 192.88.99.1 under the NAT64 local-use prefix.
    [
      "NAT64 local-use carrying a blocked IPv4",
      "http://[64:ff9b:1:c058:63:100::]/",
    ],
    [
      "NAT64 local-use carrying anything at all",
      "http://[64:ff9b:1:5db8:d822::]/",
    ],
  ])("refuses the %s under the private-network opt-in", async (_label, url) => {
    const { spy } = stubFetch(() => new Response("should not be reached"));
...
describe("URL fetch policy: log hygiene", () => {
  it("does not put a presigned signature or credentials in the log", async () => {
    const { spy } = stubFetch(() => new Response("should not be reached"));
    const log = makeLog();
    const url =
    ...
    // A content URL is client-supplied and routinely carries a signature or
    // credentials, neither of which belongs in a log sink.
    ...
    // The host and path still have to be there to be actionable.
    expect(line).toContain("169.254.169.254/latest/meta-data/");
  });
...
describe("URL fetch policy: transport hazards", () => {
  ...
  it("does not leak a redirect Location's query into the hop suffix", async () => {
    ...
    expect(await fetchUrlBytes("http://public.example/start", log)).toBeNull();
    const line = String(log.error.mock.calls[0][0]);
    expect(line).toContain("at redirect target http://127.0.0.1/internal");
  ...
  it("treats a zero status as a transport failure, not an opaque redirect", async () => {
    ...
    globalThis.fetch = (async () => errored) as unknown as typeof fetch;
    const log = makeLog();
  ...
  it("does not leak the URL through a runtime error message", async () => {
    ...
    expect(line).not.toContain("deadbeefcafe");
    expect(line).toContain("public.example/a.png");
  });
